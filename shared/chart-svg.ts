// Pure SVG chart generator — no Vue dependency.
// Used by both the frontend composable and the server-side API.

export interface ChartStarPoint {
  date: string;
  count: number;
}
...
export function emptyChartSvg(): string {
...
export function generateChartSvg(
  ...
  const maxStars =
    repos.length > 0
      ? Math.max(
          ...repos.map((r) => (r.stars.length > 0 ? (r.stars.at(-1)?.count ?? 0) : 0)),
        )
      : 0;
